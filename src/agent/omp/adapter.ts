...
export class OmpAdapter implements OmpRunEngine {
  ...
  async start(opts: AgentRunOptions): Promise<AgentRun> {
    ...
    const sessionId = resumableSessionId(this.sessionDir, opts.sessionId);
    if (opts.sessionId && !sessionId) {
      log.warn('session', 'resume-file-missing', { sessionId: opts.sessionId });
    }
...
function resumableSessionId(
  ...
  if (!sessionDir || !sessionId) return sessionId;
  try {
    const suffix = `${sessionId}.jsonl`;
    return readdirSync(sessionDir).some((name) => name === suffix || name.endsWith(`_${suffix}`))
      ? sessionId
      : undefined;
  } catch (err) {
    return (err as NodeJS.ErrnoException).code === 'ENOENT' ? undefined : sessionId;
  }
}
