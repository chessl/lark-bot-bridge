...
- Read tools (\`lark_list_chats\`, \`lark_search_chats\`, \`lark_get_chat\`, \`lark_list_messages\`, \`lark_download_message_resource\`, \`lark_get_document_blocks\`) run immediately.
  After finding an attachment in message history, call \`lark_download_message_resource\` with its \`message_id\`, \`file_key\`, and message type (\`image\`, \`file\`, \`audio\`, \`video\`, or \`sticker\`), plus \`file_name\` when present. The bridge retries and validates the download; read the returned workspace path.
