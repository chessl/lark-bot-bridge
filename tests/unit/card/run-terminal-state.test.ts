...
describe('terminal OMP Run state', () => {
  ...
  it('renders intermediate reasoning as Markdown', () => {
    const content = '**Planning to use tob-oncall skill**';
    const card = renderOmpReplyCard(stateFrom([{ type: 'reasoning', content }]));
      ...
      body: {
        elements: expect.arrayContaining([
          expect.objectContaining({
            ...
            elements: expect.arrayContaining([
              expect.objectContaining({ tag: 'markdown', content }),
            ]),
          }),
        ]),
      },
