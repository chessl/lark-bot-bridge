...
describe('P2P OMP Reply', () => {
  ...
  it.each([
    {
      ...
      expectedReasoning: 'EARLIER_ASSISTANT',
    },
    {
    ...
    },
    {
      ...
      expectedReasoning: 'PARTIAL_INTERRUPTED',
      unfinishedTool: true,
    },
    {
      name: 'idle timeout',
      ...
      expectedReasoning: 'PARTIAL_TIMEOUT',
      unfinishedTool: true,
    },
    {
      name: 'failure',
      ...
      expectedAnswer: '运行失败。',
      expectedReasoning: 'PARTIAL_FAILURE',
      unfinishedTool: true,
    },
  ] satisfies readonly {
    name: string;
  ...
  it('projects ordered progress without disclosing hidden OMP fields', async () => {
    ...
    expect(outbound).toContain('INTERMEDIATE_ASSISTANT');
    expect(outbound).toContain('EXPLICIT_RPC_REASONING');
    expect(outbound).toContain('FIRST_ASSISTANT');
    expect(outbound).toContain('SECOND_ASSISTANT');
  ...
  it('captures the 12/13 Reasoning and 20/21 Tools boundaries with cumulative totals', async () => {
    ...
    expect(JSON.stringify(twelve)).toContain('REASON_01_END');
    expect(JSON.stringify(twelve)).toContain('REASON_12_END');
    expect(JSON.stringify(twelve)).not.toContain('REASON_13_END');
    expect(JSON.stringify(twelve)).toContain('中间过程（12 条）');

    const twentyOutbound = JSON.stringify(twenty);
    expect(twentyOutbound).not.toContain('REASON_01_END');
    expect(twentyOutbound).toContain('REASON_02_END');
  ...
  it('captures exact 200-element and would-be 201-element controller projections', async () => {
    ...
    expect(overOutbound).not.toContain('"content":"REASON_1"');
    expect(overOutbound).toContain('REASON_2');
