...
describe('OmpAdapter process contract', () => {
  ...
  it('negotiates RPC v2, streams events, and persists the OMP session id', async () => {
    ...
    expect(record.systemPrompt).toContain('## 输入契约');
  ...
  it('passes profile, model, resume id, and image payload through RPC', async () => {
    ...
    const sessionDir = join(fake.dir, 'bridge-sessions');
    await mkdir(sessionDir);
    await writeFile(join(sessionDir, '2026-09-03T00-00-00-000Z_session-old.jsonl'), '');
    const run = await new OmpAdapter({
      binary: fake.path,
      profile: 'work',
      sessionDir,
    }).start({
  ...
  it('starts fresh when the pinned directory does not contain the resume session', async () => {
    const fake = await createFakeOmp();
    cleanup.push(fake.dir);
    const sessionDir = join(fake.dir, 'empty-bridge-sessions');
    const run = await new OmpAdapter({
      binary: fake.path,
      sessionDir,
    }).start({
      runId: 'run-missing-resume',
      prompt: 'start fresh',
