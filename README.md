...
- **Interactive controls**: use command cards in chat, or run the optional local web console to manage profiles and online bots.
...
Service commands support two deployment modes:

- By default, each profile runs as its own service.
- With `--web-ui`, one machine-wide supervisor hosts all profiles and a local web console.
...
# All profiles plus the web console
lark-bot-bridge start --web-ui
lark-bot-bridge status --web-ui
lark-bot-bridge restart --web-ui
lark-bot-bridge stop --web-ui
lark-bot-bridge unregister --web-ui
lark-bot-bridge ui
```

`lark-bot-bridge ui` opens the running console in your browser; add `--print` to print its URL instead.

Platform mapping:
- **macOS**: launchd user agent `ai.lark-bot-bridge.bot.<profile>`
- **Linux**: systemd user unit `lark-bot-bridge.bot.<profile>.service`

Per-profile daemon logs are under `~/.lark-bot-bridge/profiles/<profile>/logs/daemon/`. Supervisor logs are under `~/.lark-bot-bridge/logs/`.
...
```text
lark-bot-bridge run [--profile <name>] [--workspace <path>] [--web-ui] [-c <config>]
lark-bot-bridge ui [--print]
lark-bot-bridge ps
lark-bot-bridge kill <id|#>
lark-bot-bridge secrets <set|list|remove>
...
| `/new chat [name]` | Create a group with a new session and invite the requesting user |
...
| `/account` | View or replace the current app credentials |
...
| `/meeting ...` | Join, leave, inspect, summarize, or ask about a meeting |
| `/help` | Help card |
...
| `~/.lark-bot-bridge/logs/` | Machine-wide supervisor logs |
